...
class Question: Decodable {
    ...
    init() {
        ...
        self.question = "The seed drill was invented by which British inventor?"
        self.correct_answer = "Jethro Tull"
        self.incorrect_answers = ["Charles Babbage", "Isaac Newton", "J.J Thomson"]
    }
    ...
    private func decodeBase64(string: String) -> String? {
        if let stringData = Data(base64Encoded: string) { // Convert string to Base64 Encoded Data
            if let decodedString = String(data: stringData, encoding: .utf8) { //Decode string
                return decodedString // Return the new string
            }
        }
        return nil
    }
